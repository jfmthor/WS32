...
class DS3231:
    def __init__(self, i2c, adress=DS3231_I2C_ADDR):
        self.ds3231 = i2c
  ...
  #          raise DS3231Exception("DS3231 not found on I2C bus at %d" % DS3231_I2C_ADDR)
    ...
    def get_time(self, out=False):
        ...
        if out:
            timeUnixConvert = utime.mktime(self.convert(set_rtc)) + NTP_DELTA - TIME_ZONE
            return timeUnixConvert
    ...
    def save_time(self, ntp=0):
        if ntp:
           ...
           convertToDatetime = convertToDatetime[0:3] + (0,) + convertToDatetime[3:6] + (0,)
